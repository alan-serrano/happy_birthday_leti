document.addEventListener('DOMContentLoaded', function Validation() {
    ...
    function validateAnswer() {
        ...
        if(correctMonth === month * 1 && correctDay=== day * 1) {
            const message = 'PERDISTE A TU NOVIA';
            const iconClass = 'ambulance';

            $(setModalMessage(message, iconClass))
                .modal({
                    onApprove: ()=> {
                        const message = 'Era broma, la respuesta era correcta (Te amo)';
            ...
            countOpportunities--;
            const message = `Tienes una oportunidad más para recuperar a tu novia`;
